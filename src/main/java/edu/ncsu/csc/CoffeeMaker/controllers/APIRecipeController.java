...
public class APIRecipeController extends APIController {
     ...
     *
     * @param recipe
     *            The valid Recipe to be edited
     * @return ResponseEntity indicating success if the Recipe could be saved to
     *         the inventory, or an error if it could not be
     */   
    @PutMapping(BASE_PATH + "/recipes/{name}")
    public ResponseEntity updateRecipe(@PathVariable("name") String name, @RequestBody final Recipe recipe) { 
        try {
            Recipe existingRecipe = service.findByName(name);
            if (existingRecipe == null) {
            	 return new ResponseEntity(errorResponse("No recipe found with name " + name), HttpStatus.NOT_FOUND);
            }
            existingRecipe.updateRecipe(recipe);
            service.save(existingRecipe);
            return ResponseEntity.ok(successResponse(name + " successfully updated"));
